...
import { checkNicknameDuplication } from '../service/checkNicknameDuplication.js';
 ...
 */
export const handleNicknameDuplication = async (req, res) => {
  try {
    const { nickname } = req.params;

    // URL 파라미터 검증
    if (!nickname) {
      const errorResponse = createErrorResponse('VALIDATION_ERROR', '닉네임이 필요합니다.');
